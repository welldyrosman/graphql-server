...
const validateChapterData = [
    ...
    .isArray({ min: 1 }).withMessage("At least one chapter is required."),
  body("chapters.*.title").notEmpty().withMessage("Chapter title is required."),
  body("chapters.*.topics").optional().isArray()
    .withMessage("Each chapter must have at least one topic."),
  body("chapters.*.topics.*.title").notEmpty().withMessage("Topic title is required.")
...
router.post("/:id", validateChapterData, async (req, res) => {
        ...
        if (id && del) {
          ...
          if (topics && topics.length > 0) {
            for (const topic of topics) {
              const { id: topicId, title: topicTitle, content, publishDate, del: topicDel } = topic;
  
              if (topicId && topicDel) {
                // Delete topic
                await db.run("DELETE FROM materials WHERE MaterialID = ?", [topicId]);
              } else {
                // Add or update topic
                await dbInsertOrUpdateMaterial(chapterId, topicId, topicTitle, content, publishDate);
              }
            }
          }
        }
  ...
  async function dbInsertChapter(courseId, chapterTitle) {
    return new Promise((resolve, reject) => {
      db.run("INSERT INTO chapters (CourseID, ChapterName) VALUES (?, ?)", [courseId, chapterTitle], function (err) {
        if (err) {
          reject(err);
        }
        resolve(this.lastID);
  ...
  async function dbInsertOrUpdateMaterial(chapterId, topicId, topicTitle, content, publishDate) {
    return new Promise((resolve, reject) => {
      db.run(
        topicId
        ...
        topicId ? [topicTitle, content, publishDate, topicId] : [chapterId, topicTitle, content, publishDate],
        function (err) {
          if (err) {
            reject(err);
          }
              ...
              const getTopics = (chapterId) => {
                return new Promise((resolve, reject) => {
                  db.all(
                    ...
                    (err, topics) => {
                      if (err) {
                        console.error("Error retrieving topics (materials):", err);
                        reject(err);
                      }
                      resolve(topics);
                    }
